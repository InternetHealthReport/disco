                ...
                if True:
                    for probeID,probeTraceInfo in msmTraceInfo.items():
                        ...
                        for timeStamp,tracerouteInfo in probeTraceInfo.items():
                            ...
                            diff=(otEnd-float(timeStamp))/60/dur
                            durationsOfDeltaStart.append(diff)
                            diff=(float(timeStamp)-otStart)/60/dur
                            durationsOfDeltaEnd.append(diff)
                            ...
                            if not isTraceComplete(msmID,tracerouteIPsWithOccuranceCounts):
                                ...
                                successfullTraceroutesOutageIDToMsmID[outageID][msmID].append(diff)
